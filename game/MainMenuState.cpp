...
#include <iostream>
	...
	void MainMenuState::Init()
	{
		...
		_settings.setTexture(this->_data->assets.GetTexture("Settings Button"));
		_exit.setTexture(this->_data->assets.GetTexture("Exit Button"));

		_title.setPosition((SCREEN_WIDTH / 2) - (_title.getGlobalBounds().width / 2), _title.getGlobalBounds().height / 2);
		_playButton.setPosition((SCREEN_WIDTH / 2) - (_playButton.getGlobalBounds().width / 2), (SCREEN_HEIGHT / 2) - (_playButton.getGlobalBounds().height));
		_settings.setPosition((SCREEN_WIDTH / 2) - (_settings.getGlobalBounds().width / 2), (SCREEN_HEIGHT / 2) + (_settings.getGlobalBounds().height));
		_exit.setPosition((SCREEN_WIDTH / 2) - (_exit.getGlobalBounds().width / 2), (SCREEN_HEIGHT/1.5) + (_exit.getGlobalBounds().height));
		//_title.setPosition((SCREEN_WIDTH / 2) - (_title.getGlobalBounds().width / 2), _title.getGlobalBounds().height / 2);
